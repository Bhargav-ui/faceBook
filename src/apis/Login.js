...
export const forgotPasswordApi = (source, email) => {
  ...
  return axios.post(
    "https://www.edstaack.com/api/learning/recovery-email-send.php",
    form
  );
};

export const resetPasswordApi = (source, pword, confPword, uid, hash) => {
  const form = new FormData();
  form.append("source", source);
  form.append("recoverPwordNewPword", pword);
  form.append("recoverPwordConfirmNewPword", confPword);
  form.append("key", hash);
  form.append("uid", uid);

  return axios.post(
    "https://www.edstaack.com/api/learning/recovery-update-pword.php",
    form
  );
};
